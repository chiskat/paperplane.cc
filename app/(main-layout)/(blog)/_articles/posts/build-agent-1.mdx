...
import { Completion } from '../demos/build-agent-1/completion'
import { CompletionMarkdown } from '../demos/build-agent-1/completion-markdown'
import { BaseChat } from '../demos/build-agent-1/base-chat'
import { ServerToolUse } from '../demos/build-agent-1/server-tool-use'
import { ClientToolUse } from '../demos/build-agent-1/client-tool-use'
...
在这个项目中，为了节约成本，我使用支持 1M 上下文且兼容 OpenAI 请求格式的智谱 GLM-5.2 模型，因此需要安装以下适配器：
...
因为后续会不断开发新功能，要用到各种 AI 组件，所以直接一次性安装完全。
...
也可以写成 `stopWhen: stepCountIs(2)`，但这样会遇到一些问题：有时，AI 会在发起一轮工具调用后，返回文字：“考虑到美国有夏令时，我再次以夏令时的时区偏移查询一遍”，然后开始第二轮工具调用，此时 `stepCountIs(2)` 就不够用了，会导致这轮工具调用后 AI 无法总结回答。

实际开发中，Agent 可能还会执行更多的步骤，因此 `stopWhen: stepCountIs(5)` 的值会调的比较大，例如 `50`；前端方面，也会开启 `sendAutomaticallyWhen`，通过前后端同时采取措施，避免工具调用作为最后一条消息的情况。
