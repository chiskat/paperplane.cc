...
old_filename: TypeScript 中的 Utility Types、intrinsic 和 infer
date: 2023-10-19 16:00:00
tags: [JS, TS, Cheatsheet]
categories: [JS]
---

本篇介绍标题中提到的三个关键词。笼统来说，它们的作用大体上可以简介为：

- Utility Types 中文叫 “实用类型”，它们是由 TS 内置提供的一系列类型工具，用于实现一些类型操作；
- `intrinsic` 是表示 “占位” 意义的关键词，有些复杂类型必须由类型系统内部来实现，所以放这么一个关键词来占位；
- `infer` 表示 “推断”，可以理解为中学数学课解方程时使用的 “未知数”，也就是把某个类型当做 “未知数”。
...
- `ConstructorParameters<T>`：同 `Parameters<T>` 功能类似，但它接受的 `T` 应该是某个类的构造函数，也就是类似于 `<typeof MyClass>` 这样；
- `InstanceType`：同 `ReturnType<T>` 功能类似，但它接受的 `T` 应该是某个类的构造函数；
- `ThisType<T>`：得到一个对象，此对象上的任何方法中的 `this` 均为 `T`，**此实用类型已被取代**。

以下是新语法，取代了 `ThisType<T>`：
