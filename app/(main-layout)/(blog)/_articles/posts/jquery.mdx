---
title: jQuery API 备忘
old_filename: jQuery API 备忘
date: 2019-02-02 10:00:00
tags: [JS, Cheatsheet]
categories: [JS]
---

jQuery 现在已经几乎不再用了，但是它的 API 设计值得学习；本文仅做备忘用，方便维护老项目时查询。
...
- `.map(cb)`，回调参数 `function(index, el)`
  同数组的 `map`，但此方法最终返回一个新的 jQuery 对象
...
- `.data(key)` / `.data()`
  ...
  - 初始值可以从 HTML 标签的 `data-*` 属性中获取，**但获取一次后即被记住，后续修改此处值不会同步**
- `.data(key, value)`
...
- `$.get(url [, data] [, success] [, dataType])` / `$.post(url [, data] [, success] [, dataType])`
  ...
  - `url` 是请求地址，`data` 是发送的数据，`success` 是回调，`dataType` 是期望的返回类
...
- `$.ajaxPrefilter([dataType,] handler)` 在每个 AJAX 发出前都会触发该 `handler` 回调，这个回调函数可以修改请求参数
