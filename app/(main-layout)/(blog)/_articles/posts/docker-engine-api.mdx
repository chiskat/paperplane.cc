...
old_filename: Docker Engine API 通过 HTTP 接口来控制 Docker
date: 2022-10-30 10:00:00
tags: [Docker, DevOps]
categories: [Docker]
---

Docker Engine API 让我们可以通过 HTTP 请求来获取容器的状态、执行相关的 Docker 命令，它提供 RESTful 的 HTTP 接口，返回 JSON 格式数据，便于我们使用 Node.js 等程序进行访问。
...
**第一种方式：**
Docker 支持 unix、tcp、fd 这三种协议的通讯，但默认并不是全部可用的；我们可以修改 dockerd 的启动指令，令其开放 tcp 协议的通讯。
