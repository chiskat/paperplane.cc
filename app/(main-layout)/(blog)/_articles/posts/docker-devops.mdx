...
old_filename: Docker 以及部署 DevOps 工具时常见的问题
...
将 Docker run 转化为 Docker Compose：https://tools.paperplane.cc/docker-run-to-docker-compose-converter/ （由我部署）
...
选择三：
使用 PaperPlane.cc Docker 源镜像：
https://docker-mirror.p01.cc

<br />

使用第三方源镜像的方法：编辑 `daemon.json`，这样配置：

```json
{
  "registry-mirrors": ["源镜像地址写在这里", "支持多个", "..."]
}
```
...
通常来说，对象写法比较好，因为 IDE 可以给出文本高亮，便于区分键和值。

但是，上面的 `SOME_VALUE` 这种空值写法情况，二者有区别：对象写法的 `SOME_VALUE` 是空值，而数组写法的 `SOME_VALUE` 是**从宿主机运行环境透传**。这种情况下，数组写法功能性更强。
...
如果你经常构建镜像，这个缓存的大小可能达到数十个 GB，非常推荐清理。
...
SSH 的 Git 克隆地址通常是这样的：`git@p01.cc:chiskat/paperplane-blog.git`，其中指定了用户名 `git`，因此我们可以实现一个程序，判断连接到主机的用户名，如果叫 `git`，便将它转发给 Git 服务。
...
::: warning 注意 Host 的写法
我这里写的是域名，因为我给数据库容器添加了 hosts；正确做法是写成 `localhost` 或者 `127.0.0.1`。
:::
...
**如果数据库就在这台服务器上，你可以把 “Host” 直接填写 `localhost`；**
如果数据库部署在内网其他机器上，改成对应的 IP 或域名即可。
...
Docker 允许容器进行端口映射，映射的端口直接对外网开放，这个过程不需要手动调整系统内置的防火墙。
这是因为，安装 Docker 后，它会接管本机的 iptables / nftables 防火墙，根据用户启动的容器来动态增删规则，容器的端口映射可以直接生效。

Docker 给用户预留了名为 `DOCKER-USER` 的防火墙规则链，Docker 不会调整这里的规则，用户可以在这里添加一些前置规则，流量在进入 Docker 内部的网关之前，会先经过此规则链处理。
