...
old_filename: AMD、CommonJS 和 ES Module 模块化的区别
...
CommonJS 使用的加载方式可以参考 [cnblog 博客上的介绍](https://www.cnblogs.com/littlebirdlbw/p/5670633.html)，以及 [CommonJS官网](http://www.commonjs.org/)，还可以参考《深入浅出 Node.js》一书。
:::

::: tab [AMD]
AMD 全名 Asynchronous Module Definition，最初是由 [require.js](https://requirejs.org/) 提出的，它是一种浏览器端的模块化异步加载约定。

有关 AMD 的介绍，阮一峰写了一些 [介绍文章](http://www.ruanyifeng.com/blog/2012/10/javascript_module.html)
:::
...
ES Module 的加载方式可以在 [ES6 入门](http://es6.ruanyifeng.com/) 中查看。
:::
...
::: warning 再次强调
浏览器不支持 CommonJS，我们写的导入语句，其实就是给 Webpack 这类构建工具 “声明一下”，然后 Webpack 就去准备动态加载的 JS 了，并不是真正意义上的模块化。
:::

前端代码使用 CommonJS 时，要注意以下几点：

- 代码在打包编译阶段就通过 `require()` 确定依赖关系，依赖的文件都会进入打包，并不是运行的时候再去引入模块；
- 因为代码最终是打包成一个 JS 来执行的，因此导出的变量不具备 Node.js 那样的缓存机制；
...
- `sideEffects`： 只对 Webpack 之类的打包工具有用，用于在 TreeShaking 时指定有副作用的文件。
...
> 副作用的定义是：对于一个模块而言，在导入它时就会直接产生影响，而并不是只在使用到它导出的内容。
