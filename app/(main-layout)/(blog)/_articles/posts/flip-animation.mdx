...
这种过渡动画名为 “FLIP 动画”。至少早在 2015 年就有介绍 FLIP 动画的文章了，可以 [点击链接](https://aerotwist.com/blog/flip-your-animations/) 前往阅读。
...
正是因为这个原因，即使去掉 FLIP 动画，也只是没了 “归位” 的过程，看上去比较突兀而已，不会出错。
...
- 步骤 ①：假设一个元素需要从 A 位置移动到 B 位置，那么我们在移动之前，先计算出 A 位置坐标并存储；
- 步骤 ②：执行 DOM 变动；
...
1. 想让浏览器产生过渡动画，必须在两次 “计算样式” 时机让浏览器检测到样式的变更；
2. 直接使用 `setTimeout()` 来修改样式是可行的，但它的执行时机太晚，从下一帧开始 “计算样式” 时浏览器才能检测到样式变更；
3. 使用 `requestAnimationFrame()` 则可以在当前帧的 “计算样式” 和 “绘制” 之间插入执行一段代码，在这段代码中修改样式，浏览器会额外安排一次 “计算样式”，此时检测到样式变更，那当前帧的 “绘制” 阶段就会安排过渡动画。
...
每次点击按钮，必须调用 `changeDOM()` 向页面中插入一个随机宽度的白方块，此时会导致蓝色方块发生移动。
...
然后，我们实现 FLIP 动画中的 “F” 和 “L”，记录元素的开始、结束位置，也就是上面说的步骤 ①、③。
直接通过 `el.getBoundingClientRect()` 获取一个元素的上下左右四个方向相对于文档的偏移即可。
...
我们来实现这个函数 `flip`，首先，它需要接受一个 DOM 元素作为参数，获取其中所有子元素，并记录这些子元素的坐标信息。

这里，涉及到一个问题：
**元素可能被暂时移出 DOM 并插入回来，我们如何跟踪一个元素？**

在之前的例子里，元素附带有 `key` 属性，可以使用 `key` 属性来标识并跟踪元素。

::: info 恭喜你
你发现了 Vue、React 中循环渲染必须设置 `key` 属性的原因，只有这样才能正确跟踪元素。
:::
...
::: info 再次恭喜你
你发现了 `WeakMap` 被发明出来的目的，它也是为了在 JS 中跟踪元素，还可以避免内存泄漏。
:::
...
function Demo1() {
  ...
  return (
    <div>
      {list.map((item, index) => (
        <div ref={domRefs[index]} key={item}>
          {item}
        </div>
      ))}
    </div>
...
```tsx {16,41}
import { cloneElement, CSSProperties, ReactElement, useLayoutEffect, useRef, useState } from 'react'
...
> 你可能想到使用 `useMemo()` 来对 `children` 做一个 “tap” 操作，期间获取并记录子元素的坐标。
...
```tsx
<Flipper flipKey={ids.join('')}>{/* 子组件... */}</Flipper>
```
