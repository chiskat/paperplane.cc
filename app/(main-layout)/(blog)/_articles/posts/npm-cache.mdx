...
本文只考虑 CI/CD 常用的系统，例如 Ubuntu、Alpine、Node 镜像；不考虑 Windows 和 macOS。
...
# GitHub Actions 的 `setup-node` 模式
...
这是因为，**每个 GitHub Actions 的流水线插件都可以注册 “Run” 和 “Post” 两个阶段，前者是插件主要执行者，是必须的；而后者专门用于收尾，可以没有 “Post” 阶段。**
如果有多个插件，那么遵循 “先 Run 的后 Post”，类似于 “洋葱模型”。
...
- 但用户目录却是 `/github/home`，非常神奇；
  还好我们 npm 缓存写的是 `~/.npm`，如果写成绝对路径，得写成 `/github/home/.npm`，非常容易写错；
