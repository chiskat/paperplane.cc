---
title: 为 AI 搭建 HTTP 代理与 Serverless 请求转发
old_filename: HTTP 代理与 Serverless 请求转发
date: 2025-06-13 10:00:00
tags: [DevOps, Node, AI]
categories: [DevOps]
---

我原先一直通过 [OpenRouter](https://openrouter.ai/) 购买和使用 AI 服务，但 AI 服务提供商往往会有服务地区限制，例如 OpenAI 不对亚洲大部分地区提供服务；而 OpenRouter 作为代理商，也遵守此规则透传用户 IP。

::: info Tips

现在 OpenRouter 会检测付款方式中的国家，对中国用户不开放 OpenAI、Claude 等模型，不推荐使用。

推荐使用 Vercel AI Gateway 或 Cloudflare AI Gateway。相比于 OpenRouter 而言，它们不收取额外费用；此外，它们对用户地区的限制也更少。
...
- 使用国内的 AI 中转商（[编程工具中转商合集](https://www.getcheapai.com/zh-cn)，[AI 中转商合集](https://www.aiapipk.com/)）：不稳定、会跑路、可能以次充好；
- 选购欧美云服务器作代理：一劳永逸，但成本太高；<br />（很多便宜的云服务供应商的 IP 已经被严重滥用了，无法用做 AI 中转）
- 使用云服务的 Serverless 服务来转发：成本低，按量计费，非常推荐；<br />这也是我使用的方式（之一），本文从零开始介绍这种 Serverless 代理请求的原理和搭建方式。
...
再举一个 JS 库的例子，在 Node.js 运行的 `axios` 就支持代理，需要安装 `https-proxy-agent`；
用法是这样的：
...
工具配置了代理后，请求是发给代理服务器的，不会直接发送给目标；
以下示例中，我们假设请求要发送给 `http(s)://target.com`，配置了代理服务器 `proxy.com`。

对于 HTTP 和 HTTPS 的请求，代理的实现方式存在区别，先来讲 HTTP：
...
可以看出，原本首行只有子路径，现在是一个完整 URL；
代理服务器收到后，就能意识到这是一个代理请求，便会把请求转发给目标，收到响应后回传给客户端。

注意，代理服务器在转发请求时，可能会添加一些额外的请求头；在回传响应时，可能也会添加一些响应头。
...
从上面的例子可以看出，HTTP 和 HTTPS 的代理是不一样的。
如果用 Node.js 来搭建搭理，也是需要针对这两种情况分别处理。
...
如果你拥有一台 Node.js 服务器，那么可以尝试这种方法。
但是海外云服务器的成本较高，更优的选择应该是 Serverless 云函数。
...
记得修改上文中的用户名、密码、端口号，用户名和密码是数组格式，因此可以设置多个。
这里的 `allowTransparent` 可以开启，这会启用 “透明模式”，如果你的请求工具或目标无法正常运作，可以尝试开启它。

# Serverless 转发

使用云服务器的成本太高。现在很多云服务厂商例如 Cloudflare、腾讯云、阿里云，提供 Serverless 服务，且往往伴随着较大的免费额度。
而转发 AI 请求这类简单需求，使用 Serverless 来做再简单不过，接下来介绍这些方法。
...
这种方式，这个第三方服务对请求和响应完全可见，安全性较差；**如果你用它转发 AI 接口或者 GitHub API，它的部署方甚至可以直接窃取你的 Token。**
所以，更好的选择是自己来搭建，这样就具备了安全性；更重要的是，自己搭建可以使用 Serverless，即使没有云服务器也可以实现，既简单又便宜。
...
export default {
  async fetch(request, env, ctx) {
    ...
    const reqHeaders = Object.fromEntries(
      request.headers.entries().filter(([k]) => !k.startsWith('cf-'))
    )
...
域名使用腾讯云的云函数 URL 即可；如果有需求，也可以绑定自己的域名。
注意大陆地区似乎必须给域名备案，香港地区或国外的云函数域名可以不用备案。

注意，腾讯云的云函数也会给请求添加很多请求头，大部分以 `x-scf-` 和 `x-cube-` 开头，这些请求头甚至还包含腾讯云账户 ID 等敏感信息，一定要去除，代码中已经提供了去除的逻辑；
此外，还有 `x-real-ip` 和 `x-forwarded-for` 两个请求头，似乎是 `express` 加上的，代码中也已做去除。

---

此后，可以使用此云函数的 URL 进行代理访问。

测试起来也很简单，在无法访问 Google 的网络环境下，运行以下命令：
...
但是，把网址前面加上自己的云函数 URL，再试一次：

::: code-group

```bash [Linux 或 macOS]
curl https://xxxxxxxxx.na-siliconvalley.tencentscf.com/https://www.google.com
```

```powershell [Windows]
Invoke-WebRequest -Uri "https://xxxxxxxxx.na-siliconvalley.tencentscf.com/https://www.google.com"
```

:::

这时就会发现，请求成功了。
Google 会识别来自 curl 的请求，返回内容是一个简单的 `<a>`，这是正常现象，添加浏览器 UA 后可以获取完整的首页 HTML。

## 腾讯云云函数使用 express 代理中间件

上面的例子中，请求目标 URL 可以自己随便填写。
但实际上，我们可以只代理针对某个特定目标的请求，这个目标是写死的，例如是 OpenRouter。

因为大部分 AI 工具都允许我们自定义 Endpoint，我们只需要修改一下 Endpoint，从 OpenRouter 改为我们自己的云函数 URL 即可，AI 工具的请求先发给云函数，云函数转发给 OpenRouter。

这里可以用到 `express` 和 `http-proxy-middleware`，这个中间件的行为就如同上面描述的一样，且非常好配置，只需要配置目标 URL 和是否更换 `Origin` 即可。
