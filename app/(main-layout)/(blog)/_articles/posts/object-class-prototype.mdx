...
old_filename: Object、Class、prototype 和 __proto__
...
2. 然后把 `Person` 的 `.prototype` 对象复制一份拿来当做自己的 `.prototype`，这样就能继承 `Person` 上的静态方法；
3. 因为上一步操作的把 `.prototype` 上的 `.constructor` 也一起改了，这个属性表示该原型的构造函数是哪个；`Student` 类的构造器肯定是 `Student` 构造函数本身，因此将它设置为 `Student`；
   如果省略这一步，别人对 `Student` 对象使用 `Object.create()` 构造出来的对象会是一个 `Person` 对象。
