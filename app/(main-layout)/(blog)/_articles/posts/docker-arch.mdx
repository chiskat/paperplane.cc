...
旧版本的 Docker 默认使用 `overlay2` 作为镜像存储引擎，我们需要切换成 `containerd`。

**切换成 `containerd` 模式会导致之前所有镜像无法使用。** 所以这一步越趁早越好，最好在服务器刚初始化好，安装好 Docker，就马上做这一步。
