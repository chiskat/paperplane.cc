...
old_filename: 地址栏 Tab 键搜索与 OpenSearch 协议
...
浏览器的地址栏中，很多网站只需要输入网址（的开头），地址栏便会显示一个 “使用 ... 搜索” 的提示，如图：
...
不同的网站，搜索时使用的 url 和查询参数互不相同，例如：谷歌搜索使用的是 `/search?q=关键词`，而百度是 `/s?wd=关键词`，所以网站的开发者必须通过一些约定，把搜索功能的规则传递给浏览器。
