...
- `ramda` 可以避免修改对象的状态，避免引起不必要的更新，造成性能浪费，避免产生数据不一致的情况；
- `ramda` 提供了类似不可变对象的操作，可以保证修改操作完成后返回新对象，避免无法触发更新；
...
- 对于 `ramda` 中不支持 `Promise` 的函数，`ramda-adjunct` 添加了支持 `Promise` 的函数，这些函数以 `P` 作为后缀，例如：`reduceP()` 提供了数组的异步折叠计算支持；`allP()` 可以等待数组中所有 `Promise` 类型已完成再返回结果；
- 对于对象数据：
...
- 这些函数默认是不接受配置参数的，如果需要传入配置参数（例如 `format()` 的第三个参数），请使用这些函数带有 `WithOptions` 名称后缀的版本（例如 `formatWithOptions()`），此时配置参数位于第一位。
...
**有关 Y 组合子的诞生过程，以及详细的使用说明和拆解，可以参考我的 [这一篇博文](https://paperplane.cc/post/y-combinator)，它是互联网上为数不多的使用纯 JS 推导而不是使用 lambda 演算推导的文章。**
...
强烈建议，阅读我的 [这一篇博文](https://paperplane.cc/post/y-combinator)，了解 Y 组合子的构建方式、原理。
