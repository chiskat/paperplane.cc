...
import { ModelNoTransition } from '../demos/animation-compose/model-no-transition'
import { ModelTransitionAppear } from '../demos/animation-compose/model-transition-appear'
import { ModelTransitionPair } from '../demos/animation-compose/model-transition-pair'
import { ModelTransitionDone } from '../demos/animation-compose/model-transition-done'

建议先阅读 [《FLIP 动画与实现原理》](https://paperplane.cc/post/flip-animation) 这篇博文。

本文从实战开发入手，分析在 React 中控制过渡动画的 “调度” 操作；下文还会介绍 `react-transition-group` 库，它用于**组织**过渡动画。

# 用 React 简单实现模态框

我在 [《CSS 开发的优良习惯、技巧与避坑》](https://paperplane.cc/post/css-tips) 这篇博文中提到一个 “完善” 的模态框应该是怎样的。本文只考虑过渡动画这方面，不考虑其他功能是否完善。
...
而且，要在模态框关闭时，等待若干毫秒，等到模态框消失的渐出过渡动画播放完毕，再把 DOM 移除掉。
...
上述代码中，相比于之前封装的 hook，存在着几处细节优化：

- 细节 ①，使用 `useLayoutEffect()` 可以让过渡从这一帧的绘制时就开始执行，具体原理请参阅 [《FLIP
  动画与实现原理》](https://paperplane.cc/post/flip-animation) 一文；
...
使用这个组件后，我们可以从 CSS 文件中 `.mask` 样式中移除掉 `transition` 和
`opacity`，改为使用组件属性来提供这些样式。 只需要把模态框放置于组件中，代码如下：
...
function Dialog(props: IDialogProps) {
  ...
  return createPortal(
    <ToggleTransition state={show} transition="opacity 500ms" inactiveStyle={{ opacity: 0 }}>
      <div className="mask" onClick={onClose}>
        <div className="dialog">{children}</div>
      </div>
    </ToggleTransition>,
...
- 组件 DOM 被插入时带着 `v-enter-from` 类名，插入 DOM 后的下一帧这个类名就没了，变成了 `v-enter-to`，直到过渡动画完成；
  DOM 被移除时也是一样，在开始过渡时会被设置 `v-leave-from` 类名，从下一帧开始这个类名就没了，变成了 `v-leave-to` 直到过渡动画完成；
- 速记：开始过渡时 DOM 出现一瞬间是 `-active` 和 `-from` 类名，然后从下一帧开始直到过渡结束都是 `-active` 和 `-to` 类名；总之 `-from` 只有一瞬间。

::: info 说明

这里的 “帧” 指的是 “动画帧”，也就是 `requestAnimationFrame` 里的这个 `Frame`，它在浏览器 “绘制” 画面之前插入，因此这个瞬间是不可见的。
...
上面提到了，过渡全程都会有 `-active` 类名，它被设计出来的目的是，把专用于过渡的样式和组件原始样式隔离开，避免污染：
...
<template>
  <TransitionGroup style="display: flex" tag="div">
    <div class="box" v-for="item of list" :key="item">{{ item }}</div>
  </TransitionGroup>
...
再次建议没看过 [《FLIP 动画与实现原理》](https://paperplane.cc/post/flip-animation) 这篇博文的读者点击阅读。

# 使用 `react-transition-group` 组织过渡动画

::: info 说明
实际上，使用 `react-transition-group` 调度过渡动画并不高效，但它作为一个专注于调度过渡动画的 React 库，很契合本文的主题，因此对它进行介绍；实际开发中推荐使用 `motion` 等功能更强大的库。
:::
...
你可能注意到了，这个组件并没有提供一个类似于 `v-enter-from` 和 `v-leave-from` 这种只在绘制前生效一瞬间的类名。

它和 Vue 的 `<Transition>` 的区别如下图所示：

![](../images/image-20241204023712691.png)

图中的红色区域，也就是 Vue 提供的这种只持续一帧的 `-from` 类名，在 `<CSSTransition>` 中却并不提供，这导致使用此组件时，样式上需要换一种思路。

在 Vue 的 `<Transition>` 组件中，可以通过在 `v-enter-from` 和 `v-enter-to` 中提供不同的样式，来激活过渡动效；
例如，`v-enter-from` 的高度为 0，`v-enter-to` 的高度为 300px，这样只要此组件出现，就会产生一个向下展开的效果。

在 React 生态的 `<CSSTransition>` 组件中，我们只能把初始状态写在 `前缀-enter` 里，然后再在 `前缀-enter-active` 里去 “覆盖” 掉之前的状态，达成样式变动，激活动效。
...
这个组件是通过类名来控制组件样式的，而我们提供的前缀叫 `"dialog"`；
编辑 CSS 文件，设置以下样式，前缀就必须用 `dialog`：
