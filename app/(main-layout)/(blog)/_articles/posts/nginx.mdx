---
title: Nginx 配置模板与解读
old_filename: Nginx 配置模板与解读
date: 2022-06-28 18:00:00
tags: [DevOps, DOC, Cheatsheet, Scaffold]
categories: [DevOps]
---
...
注意，如果你的 Nginx 的上游服务本身只支持特定的 HTTP 协议版本，那么要加一行：
...
否则 WebSocket 可能会一直出错，却不会显示原因。

# AI 流式输出优化

前两项，优化流式输出的流畅度；后门几项避免流式输出超时；
推荐把这些配置放在 `/api` 等纯数据接口处，直接放在网站根级 `/` 也行，但不如前者。
