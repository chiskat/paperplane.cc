...
部署后，请通过 Nginx 对外暴露访问，打开网页，输入刚设置的访问密码进入主界面，准备下一步登录账号。
...
我们可以在域名后面，添加 `/v1` 后缀，这便是 OpenAI 兼容接口；也可以使用 `/v1/messages` 这个路径，来使用 Anthropic 接口格式；甚至还可以使用 Gemini 的接口格式。
我们使用接口时，不论哪一种格式，Antigravity Manager 会自动适配；甚至，使用了 Antigravity 不提供的模型，工具也可以帮我们映射为当前可用的模型。
...
这样即可调用 Gemini 等模型。我的实例实测如下图：

![](../images/image-20260608185311716.png)

我这里配置好了网络。如果你使用时，提示 400 错误，请继续阅读下文，设置网络。

## 关于额度计算

目前，Antigravity 的用量是这样计算的：

- 现在免费用户的 Gemini Pro 和 Gemini Flash 是一起算的，每 7 天重置一次免费用量；
...
Cloudflare WARP 是免费的零信任安全网络，接入后，你的流量在网络中加密传输，并通过随机的出口访问外部服务。使用它来代理 Antigravity 的请求，便可以正常使用，不再回出现 400 报错。

安装 Cloudflare WARP 特别复杂，而且对主机有严重的影响，会让主机的所有出口流量都被代理，不太值得。

好在，CaoMingJun 大神开发了 `caomingjun/warp` 这个镜像，允许我们通过 sock5 接入 WARP，只代理这部分流量。部署方式：
...
如果你的 WARP 容器名字不叫这个，记得一同修改了；配置后，可能要重启 Antigravity Manager。

::: info 注意

WARP 必须和 Antigravity Manager 接入同一个 Docker Network，才能使用容器名来互相访问，最简单方式就是把它们的配置放在同一个 `docker-compose.yml` 文件中。
...
不仅是我，很多人也遇到了 Gemini 的工具调用 400 报错，内容如下：
...
用户的输入触发了工具调用，比如搜索网络，或检索文件，此时便会触发以上报错，导致任务中断。

下图中，我的问题触发了网络搜索工具调用，Antigravity 返回了 400 报错，任务直接终止：
...
因为这个思维链的签名只在工具调用的步骤才需要，所以，只要当前发送的消息体中没有工具调用，那请求大概率会成功。
