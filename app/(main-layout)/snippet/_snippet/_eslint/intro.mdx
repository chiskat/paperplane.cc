...
ESLint 已由过去 v8 版本的 “按包名加载” 转为 v9 版本开始的 “从代码加载”，因此开始逐步废弃旧版 JSON 配置方式。
...
在 .mjs 文件中，导入导出使用 `import` 和 `export` 语句；
...
需要注意的是，ESLint 配置中的 `files` 字段仅仅是针对某部分文件覆盖规则，即使某个文件不被任何 `files` 所包含，它仍会被 ESLint 检查到；
