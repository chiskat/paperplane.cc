...
上文中，`msw` 是一个模拟的 “HTTP 服务器”。
有些时候我们需要真实 Node.js 服务器，这种配置稍有不同。

之前使用 `setupFiles` 来配置开发服务器，因为每个测试文件之间是隔离的，因此 `setupFiles` 里的初始化代码会在每个测试文件运行前都执行一次；
...
代码中需要注意的是，`globalSetup` 的代码和测试程序不处于同一个作用域，无法互相访问全局变量，因此初始化代码通过 `project.provide()` 向测试程序传递参数；
测试程序中，需要这样接收参数：
