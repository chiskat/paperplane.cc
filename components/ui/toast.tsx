...
interface ToasterProps extends Omit<ComponentProps<typeof ArkToaster>, 'toaster' | 'children'> {
  /**
   * Toaster instance
   */
...
export const Toaster = (props: ToasterProps) => {
  const { toaster: toasterInstance = baseToaster, ...rest } = props
...
export const ToastItem = (props: ToastItemProps) => {
  ...
  const toastKey = (toastData as { meta?: { toastKey?: string } }).meta?.toastKey
    ...
    >
      <div className="flex items-start gap-1.5">
        ...
        <div className="flex flex-col gap-0.5">
          <ArkToast.Title className="text-sm font-medium" data-slot="toast-title">
            {toastData.title}
          </ArkToast.Title>
...
const getToastKey = (options: Pick<BaseToastOptions, 'id' | 'title' | 'description'>): string => {
  const id = options.id ?? ''
...
export const toast = {
  ...
  success: (options: BaseToastOptions) => createWithDedupe({ ...options, type: 'success' }),
  error: (options: BaseToastOptions) => createWithDedupe({ ...options, type: 'error' }),
  warning: (options: BaseToastOptions) => createWithDedupe({ ...options, type: 'warning' }),
  info: (options: BaseToastOptions) => createWithDedupe({ ...options, type: 'info' }),
  loading: (options: BaseToastOptions) => createWithDedupe({ ...options, type: 'loading' }),
}
