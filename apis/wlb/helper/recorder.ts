...
import { getSnapshot } from './puppeteer'
import {
...
export async function wlbRecord(wlbProfile: WLBProfile) {
  ...
  const trafficSnapshot = await getSnapshot({
    url: `http://localhost:3000/wlb/traffic/${profileId}`,
    width: 650,
    height: 650,
  })
  ...
  const viewSnapshot = await getSnapshot({
    url: `http://localhost:3000/wlb/record/${id}`,
    width: 1500,
    height: 800,
  })
  const { fileUrl: imageURL } = await publicUpload(`/offwork-image/${record.id}.png`, viewSnapshot)
